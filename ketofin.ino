...
#define ACETONE_ADC_PIN A1 // ADC pin for acetone sensor

// Sensor constants
const float VB_AIR = 0.295;   // Vb(Air) baseline = 0.295V (295mV)
const float MIN_PPM = 0.0;    // Minimum PPM value
const float MAX_PPM = 20.0;   // Maximum PPM value
...
// Global variables for breath test
int lowestADC = 1023;  // Initialize to maximum possible ADC value
int highestADC = 0;
...
void setup() {
  ...
  display.setRotation(1);          // Set rotation once
...
// Function to convert ADC value to PPM using square root interpolation
...
void drawBattery() {
  // Draw battery indicator at top-left
  int x = 2, y = 2, bw = 18, bh = 8;
  display.drawRect(x, y, bw, bh, SSD1306_WHITE);
  display.fillRect(x + bw, y + 2, 2, bh - 4, SSD1306_WHITE);  // terminal
  // Show full battery level
...
void blowOutAnimation() {
  ...
  for (int t = 0; t < 50; t++) {
    ...
    // Vertical breathing bar animation
    int barX = 24;
    int barY = 50;
    int barWidth = 16;
    int barHeight = 50;
    
    // Draw outer rectangle (vertical bar)
...
float getAcetoneLevel() {
  // Use the difference between highest and lowest ADC values
  ...
  // Convert to voltage - this is Vb(Gas)
  float vb_gas = (adcValue * 5.0) / 1024.0;
  
  // Calculate ΔVOUT = Vb(Gas) - Vb(Air)
  float delta_vout = vb_gas - VB_AIR;
  float delta_vout_mv = delta_vout * 1000.0; // Convert to mV
  
  // Convert ADC to PPM using calibration data
...
void processingAnimation() {
  // Processing animation screen for 3 seconds with rotating circle
  for (int cycle = 0; cycle < 30; cycle++) { // 30 cycles for 3 seconds (100ms each)
    display.clearDisplay();
    ...
    for (int i = 0; i < numDots; i++) {
      ...
      // Make dots fade by drawing fewer dots for trailing effect
...
String getKetoStatus(float ppmValue) {
  // Determine ketosis status based on PPM value
  if (ppmValue < 1.0) return F("  None  ");
  else if (ppmValue < 5.0) return F("  Light ");
  else if (ppmValue < 10.0) return F("Moderate");
  else if (ppmValue < 15.0) return F("  High  ");
...
void displayResults() {
  ...
  // if (finalPPM < 4) {
  //   // Generate a random value between 15 and 20
  //   finalPPM = 5 + (rand() % 2); // rand() % 6 generates a random number from 0 to 5
  // }
  drawBattery();
  
  // Display "Acetone Level" title
  display.setTextSize(1);
  ...
  // Status bar based on ketosis level
  int barWidth = 40;
  int barHeight = 4;
  int barX = 10;
  int barY = 110;
  ...
  // Fill bar based on ketosis level
  int fillWidth = 0;
  if (ketoStatus == "  None  ") fillWidth = barWidth * 0.1;
  else if (ketoStatus == "  Light ") fillWidth = barWidth * 0.4;
  else if (ketoStatus == "Moderate") fillWidth = barWidth * 0.7;
  else if (ketoStatus == "  High  ") fillWidth = barWidth * 0.9;
  else fillWidth = barWidth; // Very High
...
void welcomeScreen() {
  ...
  // Center the welcome text better
...
void countdown() {
  // "Let's Start" screen with countdown
  for (int i = 15; i >= 1; i--) {
    display.clearDisplay();
    display.setTextColor(SSD1306_WHITE);
    ...
    // Display countdown with leading zero
...
void loop() {
  ...
  // Blow out phase (5 seconds) - Capture ADC values
  blowOutAnimation();
  ...
  delay(5000); // Display results for 5 seconds before restarting
}